...
/*
This generates an ADC clock using GPIO pin P8_45 (r30.0).

[VOP] Also generates the CS signal for SPI control using GPIO pin P8_43 (r30.2).
...
TOP:
  ...
  //[VOP] Unused and may be removed in future version
...
CYCLE_EVEN_H:
  ...
  //[VOP] GPIO clock pin P8_43 goes low after 4 SCLK cycles
  qble WAIT_H, CS_COUNTER, 4
  //[VOP] CS goes low 25 or 30ns after SCLK goes high, costs 2 cycles
  sub HIGH_COUNTER, HIGH_COUNTER, 2
  clr r30, 2
...
CYCLE_EVEN_L:
  ...
  //[VOP] GPIO clock pin P8_43 goes high after 20 SCLK cycles
  qblt WAIT_L, CS_COUNTER, 20
  //[VOP] CS goes high 30 or 35ns after SCLK goes low, costs 3 cycles, but force even => 4
...
WAIT_L:
  ...
  // GPIO clock pin P8_45 goes high
  set r30, 0
